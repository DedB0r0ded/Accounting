...
// =====Getters
lid_t               Transfer::id()              const { return id_; }
opt_tm              Transfer::date_time()       const { return date_time_; }
double              Transfer::conversion_rate() const { return conversion_rate_; }
bool                Transfer::is_valid()        const { return state_switch_.is_validated(); }
TransferStateSwitch Transfer::state_switch()    const { return state_switch_; }
TransferState       Transfer::state()           const { return state_switch_.get_state(); }
const string&       Transfer::err_msg()         const { return err_msg_; }
const sptr<Account> Transfer::debit_account()   const { return debit_account_; }
i32                 Transfer::debit_amount()    const { return debit_amount_;}
const sptr<Account> Transfer::credit_account()  const { return credit_account_; }
i32                 Transfer::credit_amount()   const { return credit_amount_; }


// =====Setters
void Transfer::set_id               (const lid_t& id)                         { id_ = id; }
void Transfer::set_date_time        (const opt_tm& date_time)                 { date_time_ = date_time; }
void Transfer::set_conversion_rate  (double rate)                             { conversion_rate_ = rate; }
void Transfer::set_state_switch     (const TransferStateSwitch& state_switch) { state_switch_ = state_switch; }
void Transfer::set_err_msg          (const string& msg)                       { err_msg_ = msg; }
void Transfer::set_debit_account    (const Account& account)                  { *debit_account_ = account; } // add null-check
void Transfer::set_debit_amount     (i32 amount)                              { debit_amount_ = amount; }
void Transfer::set_credit_account   (const Account& account)                  { *credit_account_ = account; } // add null-check
void Transfer::set_credit_amount    (i32 amount)                              { credit_amount_ = amount; }
