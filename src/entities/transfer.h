...
class Transfer {
 private:
  ...
  sp<Account> debit_account_;
  i32 debit_amount_;
  Currency debit_currency() const;

  sp<Account> credit_account_;
  i32 credit_amount_;
 ...
 public:
  ...
  Transfer(lid_t id, const tm& date_time, sp<Account> debit_account,
  ...
  const sp<Account> debit_account() const;
  i32 debit_amount() const;
  const sp<Account> credit_account() const;
  i32 credit_amount() const;

  // Setters
  // Add tests
  void set_id(const lid_t& id);
  void set_date_time(const Time& date_time);
